...
export function bindSpatialCanvasEvents(store) {
  ...
  document.querySelectorAll('.canvas-view').forEach(card => {
    card.addEventListener('keydown', (event) => {
      if (event.target === card && (event.key === 'Enter' || event.key === ' ')) {
        event.preventDefault();
        store.focusCanvasView(card.dataset.canvasView);
      }
    });
  });
  ...
  document.querySelectorAll('.canvas-view-head').forEach(head=>head.addEventListener('pointerdown',ev=>{if(ev.button!==0||ev.target.closest('button'))return;const card=head.closest('.canvas-view'),id=card.dataset.canvasView,view=store.state.canvas.views.find(v=>v.id===id);if(!view)return;const sx=ev.clientX,sy=ev.clientY,ox=view.x,oy=view.y;let nx=ox,ny=oy;const move=e=>{nx=ox+(e.clientX-sx)/(store.state.canvas.zoom||1);ny=oy+(e.clientY-sy)/(store.state.canvas.zoom||1);card.style.left=`${nx}px`;card.style.top=`${ny}px`;};const keydown=(e)=>{if(e.key==='Escape'){e.preventDefault();finish(false);}};const finish=(commit)=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up);window.removeEventListener('pointercancel',cancel);window.removeEventListener('keydown',keydown);if(commit)store.updateCanvasView(id,{x:nx,y:ny},'human');else{card.style.left=`${ox}px`;card.style.top=`${oy}px`;}};const up=()=>finish(true),cancel=()=>finish(false);window.addEventListener('pointermove',move);window.addEventListener('pointerup',up,{once:true});window.addEventListener('pointercancel',cancel,{once:true});window.addEventListener('keydown',keydown);}));
  document.querySelectorAll('[data-canvas-resize]').forEach(handle=>handle.addEventListener('pointerdown',ev=>{if(ev.button!==0)return;ev.stopPropagation();const card=handle.closest('.canvas-view'),id=handle.dataset.canvasResize,view=store.state.canvas.views.find(v=>v.id===id);if(!view)return;const sx=ev.clientX,sy=ev.clientY,ow=view.w,oh=view.h;let nw=ow,nh=oh;const move=e=>{nw=Math.max(240,ow+(e.clientX-sx)/(store.state.canvas.zoom||1));nh=Math.max(160,oh+(e.clientY-sy)/(store.state.canvas.zoom||1));card.style.width=`${nw}px`;card.style.height=`${nh}px`;};const keydown=(e)=>{if(e.key==='Escape'){e.preventDefault();finish(false);}};const finish=(commit)=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up);window.removeEventListener('pointercancel',cancel);window.removeEventListener('keydown',keydown);if(commit)store.updateCanvasView(id,{w:nw,h:nh},'human');else{card.style.width=`${ow}px`;card.style.height=`${oh}px`;}};const up=()=>finish(true),cancel=()=>finish(false);window.addEventListener('pointermove',move);window.addEventListener('pointerup',up,{once:true});window.addEventListener('pointercancel',cancel,{once:true});window.addEventListener('keydown',keydown);}));

  viewport.addEventListener('pointerdown',ev=>{if(ev.button!==0||ev.target.closest('.canvas-view'))return;const sx=ev.clientX,sy=ev.clientY,ox=store.state.canvas.panX||0,oy=store.state.canvas.panY||0,stage=viewport.querySelector('.canvas-stage');let px=ox,py=oy;const move=e=>{px=ox+e.clientX-sx;py=oy+e.clientY-sy;stage.style.transform=`translate(${px}px,${py}px) scale(${store.state.canvas.zoom||1})`;};const keydown=(e)=>{if(e.key==='Escape'){e.preventDefault();finish(false);}};const finish=(commit)=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up);window.removeEventListener('pointercancel',cancel);window.removeEventListener('keydown',keydown);if(commit)store.setCanvasViewport({panX:px,panY:py});else stage.style.transform=`translate(${ox}px,${oy}px) scale(${store.state.canvas.zoom||1})`;};const up=()=>finish(true),cancel=()=>finish(false);window.addEventListener('pointermove',move);window.addEventListener('pointerup',up,{once:true});window.addEventListener('pointercancel',cancel,{once:true});window.addEventListener('keydown',keydown);});
}
