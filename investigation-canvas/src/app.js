...
function renderTopbar(s) {
  ...
  return `<header class="topbar">
    ...
    <nav class="tabbar" aria-label="Workspace tabs">
      ${[['explore','Explore'],['hypotheses','Hypotheses'],['evidence','Evidence'],['provenance','Provenance']].map(([id,name]) => `<button class="tab ${s.activeTab===id?'active':''}" data-tab="${id}">${name}</button>`).join('')}
    </nav>
